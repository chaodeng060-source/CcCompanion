...
```
opia: 1 windows (created ...)
```

### E.4 attach 看 cc 起没起

> 跑 `tmux attach -t opia`, 看里面 claude 是不是在等输入。看到提示符 (`>` 或 `claude>`) 就 OK。
...
期望: `sessions` 里出现一条 `{"sid": "opia", "active": true, ...}`。
...
- 症状: ccc 发消息 sessions 列表空, 没有 opia
